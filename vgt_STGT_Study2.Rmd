...
library(brms)
library(bayestestR)
library(report) #use summary command on top of 'report' command for essential info
```
# Pavlovian eye data
...
ml.st <- lmer(gazeidx~ half*CS + version + (half + CS|id), data=filter(h1_2_tr_ter, STGT_ter=="ST"))
summary(ml.st)
ml.gt <- lmer(gazeidx~ half*CS + version + (half + CS|id), data=filter(h1_2_tr_ter, STGT_ter=="GT"))
summary(ml.gt)
...
# Bayesian categorical priors

```{r}
get_prior(riskychoice ~ STGT*cues_factor*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_stgt, family=bernoulli())
...
fullpriors = c(set_prior("normal(0,1.5)", class = "Intercept"),
              ...
              set_prior("normal(0,1.5)", class = "b", coef="orderuc_c"),
...
Full model
```{r}
full <- brm(riskychoice ~ STGT + cues_factor + EVR + STGT*cues_factor + STGT*EVR+ cues_factor*EVR + STGT*cues_factor*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_stgt,  family=bernoulli(), prior=fullpriors, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000) 
summary(full)
describe_posterior(full, test=c("p_direction"))
describe_posterior(full, centrality = "mean", test=c("p_direction"))
plot(full)
```
Null phenotype*cues & phenotype*EVR interactions

```{r}
null <- brm(riskychoice ~ STGT + cues_factor + EVR + cues_factor*EVR + STGT*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_stgt, family=bernoulli(), prior=nullpriors2w3w, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000)
summary(null)
describe_prior(null)
...
```
Bayesian dimensional model without phenotype x cues x EVR interaction
```{r}
null0 <- brm(riskychoice ~ STGT + cues_factor + EVR + STGT*cues_factor + cues_factor*EVR + STGT*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_stgt, family=bernoulli(), prior=nullpriors3w, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000) 
summary(null0)
...
Null cues
```{r}
null2 <- brm(riskychoice ~ STGT + EVR + STGT*EVR + order + Rep_centered + (EVR+Rep_centered|subj), data=data_stgt, family=bernoulli(), prior=nullpriors_nocues, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000)
summary(null2)
...
Null phenotype
```{r}
null3 <- brm(riskychoice ~ cues_factor + EVR + cues_factor*EVR + order+Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_stgt, family=bernoulli(), prior=nullpriors_no_STGT, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000)
summary(null3)
...
```{r}
null4 <- brm(riskychoice ~ STGT + cues_factor + EVR + STGT*cues_factor + STGT*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_stgt,  family=bernoulli(), prior=nullpriors_nocuesEVR, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000) 
summary(null4)
...
plot_model(ml.2,type="pred", terms=c("EVR","order","cues_factor"),pred.type="fe", grid = FALSE, colors=c( "orangered","black"))+ylab("p(Riskier prospect)")+theme_bw() +theme(plot.background = element_blank(),panel.grid.major = element_blank(),panel.grid.minor = element_blank(),legend.title = element_blank(),panel.spacing = unit(2, "lines"))
```
...
ml.uc = glmer(riskychoice ~ EVR*Gaze_index + order+ Rep_centered + (EVR+Rep_centered|subj), data=filter(data_trimmed,cues_factor=="uncued"), family = binomial,control = glmerControl(optimizer="bobyqa")) # bound optimization by quadratic approximation (BOBYQA); benefits in terms of speed
summary(ml.uc)
...
# Does risk taking on the uncued correlate with the risk-promoting effect of cues?

riskiness <- data_trimmed %>% 
  group_by(cues_factor,subj) %>% 
  summarise(riskychoice=mean(riskychoice),gaze_index=Gaze_index[1]) %>% 
  pivot_wider(names_from = cues_factor, values_from = riskychoice) %>% 
  mutate(cuedRisk = cued-uncued)

riskiness_trimmed <- riskiness %>% 
  filter(cuedRisk>-0.4)

cor.test(riskiness_trimmed$uncued, riskiness_trimmed$cuedRisk)
plot(riskiness_trimmed$uncued, riskiness_trimmed$cuedRisk)
plot(riskiness$uncued, riskiness$cuedRisk)

```

#Bayesian dimensional priors

```{r}
get_prior(riskychoice ~ Gaze_index + cues_factor + EVR + Gaze_index*cues_factor + Gaze_index*EVR+ cues_factor*EVR + Gaze_index*cues_factor*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_trimmed, family=bernoulli())
...
```{r}
full <- brm(riskychoice ~ Gaze_index + cues_factor + EVR + Gaze_index*cues_factor + Gaze_index*EVR+ cues_factor*EVR + Gaze_index*cues_factor*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_trimmed,  family=bernoulli(), prior=fullpriors, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000) 
summary(full)
describe_posterior(full, test=c("p_direction"))
describe_posterior(full, centrality = "mean", test=c("p_direction"))
...
```{r}
null <- brm(riskychoice ~ Gaze_index + cues_factor + EVR + cues_factor*EVR + Gaze_index*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_trimmed, family=bernoulli(), prior=nullpriors2w3w, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000)
summary(null)
describe_prior(null)
describe_posterior(null,test=c("p_direction"))
bridge = bayes_factor(full, null)
...
```
Bayesian dimensional model without gaze index x cues x EVR interaction

```{r}

null0 <- brm(riskychoice ~ Gaze_index + cues_factor + EVR + Gaze_index*cues_factor + cues_factor*EVR + Gaze_index*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_trimmed, family=bernoulli(), prior=nullpriors3w, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000) 
summary(null0)
describe_prior(null0)
describe_posterior(null0, test=c("p_direction"))
bridge = bayes_factor(full, null0)
...
```
Bayesian dimensional model without cues
******
```{r}

null2 <- brm(riskychoice ~ Gaze_index + EVR + Gaze_index*EVR + order + Rep_centered + (EVR+Rep_centered|subj), data=data_trimmed, family=bernoulli(), prior=nullpriors_nocues, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000)
summary(null2)
describe_prior(null2)
describe_posterior(null2, test=c("p_direction"))
bridge = bayes_factor(full, null2)
...
Bayesian dimensional model without gaze index
```{r}

null3 <- brm(riskychoice ~ cues_factor + EVR + cues_factor*EVR + order+Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_trimmed, family=bernoulli(), prior=nullpriors_no_gaze_index, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000)
summary(null3)
describe_prior(null3)
describe_posterior(null3,test=c("p_direction"))
bridge = bayes_factor(full, null3)
...
Bayesian dimensional model without cues x EVR
```{r}
null4 <- brm(riskychoice ~ Gaze_index + cues_factor + EVR + Gaze_index*cues_factor + Gaze_index*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_trimmed,  family=bernoulli(), prior=nullpriors_nocuesEVR, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000) 
summary(null4, waic=TRUE)
summary(null4)
describe_prior(null4)
describe_posterior(null4)
bridge = bayes_factor(full, null4)
bridge
```

#Bayesian dimensional priors from study 1
```{r}

fullpriors = c(set_prior("normal(-0.67,0.17)", class = "Intercept"),
              set_prior("normal(-0.72,0.22)", class = "b", coef="cues_factoruncued"),
              set_prior("normal(-0.11,0.44)", class = "b", coef="cues_factoruncued:EVR"),
              set_prior("normal(3.53,0.32)", class = "b", coef="EVR"),
              set_prior("normal(0.10,0.27)", class = "b", coef="Gaze_index"),
              set_prior("normal(0.79,0.4)", class = "b", coef="Gaze_index:cues_factoruncued"),
              set_prior("normal(0.91,0.79)", class = "b", coef="Gaze_index:cues_factoruncued:EVR"),
              set_prior("normal(0.03,0.54)", class = "b", coef="Gaze_index:EVR"),
              set_prior("normal(0,1.5)", class = "b", coef="orderuc_c"),
              set_prior("normal(0.07,0.02)", class = "b", coef="Rep_centered"))
fullpriors

nullpriors3w = c(set_prior("normal(-0.67,0.17)", class = "Intercept"),
              set_prior("normal(-0.72,0.22)", class = "b", coef="cues_factoruncued"),
              set_prior("normal(-0.11,0.44)", class = "b", coef="cues_factoruncued:EVR"),
              set_prior("normal(3.53,0.32)", class = "b", coef="EVR"),
              set_prior("normal(0.10,0.27)", class = "b", coef="Gaze_index"),
...
nullpriors2w3w = c(set_prior("normal(-0.67,0.17)", class = "Intercept"),
              set_prior("normal(-0.72,0.22)", class = "b", coef="cues_factoruncued"),
              set_prior("normal(-0.11,0.44)", class = "b", coef="cues_factoruncued:EVR"),
              set_prior("normal(3.53,0.32)", class = "b", coef="EVR"),
              set_prior("normal(0.10,0.27)", class = "b", coef="Gaze_index"),
...
nullpriors_nocues = c(set_prior("normal(-0.67,0.17)", class = "Intercept"),
              set_prior("normal(3.53,0.32)", class = "b", coef="EVR"),
              set_prior("normal(0.10,0.27)", class = "b", coef="Gaze_index"),
              set_prior("normal(0.03,0.54)", class = "b", coef="Gaze_index:EVR"),
              set_prior("normal(0,1.5)", class = "b", coef="orderuc_c"),
...
nullpriors_nocuesEVR = c(set_prior("normal(-0.67,0.17)", class = "Intercept"),
              set_prior("normal(-0.72,0.22)", class = "b", coef="cues_factoruncued"),
              set_prior("normal(3.53,0.32)", class = "b", coef="EVR"),
              set_prior("normal(0.10,0.27)", class = "b", coef="Gaze_index"),
              set_prior("normal(0.79,0.4)", class = "b", coef="Gaze_index:cues_factoruncued"),
...
nullpriors_no_gaze_index = c(set_prior("normal(-0.67,0.17)", class = "Intercept"),
              set_prior("normal(-0.72,0.22)", class = "b", coef="cues_factoruncued"),
              set_prior("normal(-0.11,0.44)", class = "b", coef="cues_factoruncued:EVR"),
              set_prior("normal(3.53,0.32)", class = "b", coef="EVR"),
              set_prior("normal(0,1.5)", class = "b", coef="orderuc_c"),
...
# Bayesian dimensional full model with priors from Study 1
```{r}
full <- brm(riskychoice ~ Gaze_index + cues_factor + EVR + Gaze_index*cues_factor + Gaze_index*EVR+ cues_factor*EVR + Gaze_index*cues_factor*EVR + order+ Rep_centered + (EVR+cues_factor+Rep_centered|subj), data=data_trimmed,  family=bernoulli(), prior=fullpriors, save_pars = save_pars(all = TRUE), sample_prior = TRUE, iter = 10000) 
summary(full)
describe_posterior(full, test=c("p_direction"))
